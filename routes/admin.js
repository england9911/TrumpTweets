...
router.get('/orders', common.restrict, function(req, res, next){
	...
	// Top 10 products
    common.dbQuery(db.orders, {}, {'orderDate': -1}, 10, function(err, orders){
        if(err){
...
// Admin section
router.get('/orders/bystatus/:orderstatus', common.restrict, function(req, res, next){
    var db = req.app.db;
    ...
    var regex = new RegExp(['^', req.params.orderstatus, '$'].join(''), 'i');
    common.dbQuery(db.orders, {orderStatus: regex}, {'orderDate': -1}, 10, function(err, orders){
        if(err){
            console.info(err.stack);
        }
        res.render('orders', {
...
router.get('/order/view/:id', common.restrict, function(req, res){
    var db = req.app.db;
    db.orders.findOne({_id: common.getId(req.params.id)}, function (err, result){
        if(err){
            console.info(err.stack);
        }
        var productOptions = '';
...
router.get('/orders/filter/:search', common.restrict, function(req, res, next){
    ...
    var ordersIndex = req.app.ordersIndex;
    ...
    common.dbQuery(db.orders, {_id: {$in: lunrIdArray}}, null, null, function(err, orders){
        if(err){
            console.info(err.stack);
        }
        res.render('orders', {
...
router.get('/order/delete/:id', common.restrict, function(req, res){
	...
	// remove the article
    db.orders.remove({_id: common.getId(req.params.id)}, {}, function (err, numRemoved){
        if(err){
            console.info(err.stack);
        }
		// remove the index
...
router.post('/order/statusupdate', common.restrict, function(req, res){
    var db = req.app.db;
    db.orders.update({_id: common.getId(req.body.order_id)}, {$set: {orderStatus: req.body.status}}, {multi: false}, function (err, numReplaced){
        if(err){
            console.info(err.stack);
        }
...
router.get('/products', common.restrict, function(req, res, next){
    var db = req.app.db;
	// get the top results
    common.dbQuery(db.products, {}, {'productAddedDate': -1}, 10, function(err, topResults){
        if(err){
            console.info(err.stack);
        }
        res.render('products', {
...
router.post('/product/addtocart', function(req, res, next){
    var db = req.app.db;
    var productQuantity = req.body.productQuantity ? parseInt(req.body.productQuantity) : 1;
    ...
    db.products.findOne({_id: common.getId(req.body.productId)}, function(err, product){
        if(err){
        ...
        if(product){
            ...
            var cartIndex = _.findIndex(req.session.cart, findDoc);
            if(cartIndex > -1){
...
router.post('/product/updatecart', function(req, res, next){
    var db = req.app.db;
    var cartItems = JSON.parse(req.body.items);
    var hasError = false;

    async.eachSeries(cartItems, function iteratee(cartItem, callback){
        var productQuantity = cartItem.itemQuantity ? cartItem.itemQuantity : 1;
        if(cartItem.itemQuantity === 0){
            // quantity equals zero so we remove the item
            req.session.cart.splice(cartItem.cartIndex, 1);
            callback(null);
        }else{
            db.products.findOne({_id: common.getId(cartItem.productId)}, function(err, product){
                if(err){
...
router.post('/product/removefromcart', function(req, res, next){
    ...
    async.each(req.session.cart, function iteratee(item, callback){
        if(item){
...
router.get('/products/filter/:search', common.restrict, function(req, res, next){
	...
	// we search on the lunr indexes
    common.dbQuery(db.products, {_id: {$in: lunrIdArray}}, null, null, function(err, results){
        if(err){
...
router.post('/product/insert', common.restrict, function(req, res){
    ...
    db.products.count({'productPermalink': req.body.frmProductPermalink}, function (err, product){
        if(err){
            console.info(err.stack);
        }
...
router.get('/product/edit/:id', common.restrict, function(req, res){
    ...
    common.getImages(req.params.id, req, res, function (images){
        db.products.findOne({_id: common.getId(req.params.id)}, function (err, result){
            if(err){
                console.info(err.stack);
            }
            var options = {};
...
router.post('/product/update', common.restrict, function(req, res){
    ...
    db.products.findOne({_id: common.getId(req.body.frmProductId)}, function (err, product){
        if(err){
            console.info(err.stack);
        }
        db.products.count({'productPermalink': req.body.frmProductPermalink, $not: {_id: common.getId(req.body.frmProductId)}}, function (err, count){
            if(err){
                console.info(err.stack);
            }
            if(count > 0 && req.body.frmProductPermalink !== ''){
                ...
                // redirect to insert
                res.redirect('/edit/' + req.body.frmProductId);
            }else{
                common.getImages(req.body.frmProductId, req, res, function (images){
                    var productDoc = {
                        productTitle: req.body.frmProductTitle,
                        productDescription: req.body.frmProductDescription,
                        productPublished: req.body.frmProductPublished,
                    ...
                    db.products.update({_id: common.getId(req.body.frmProductId)}, {$set: productDoc}, {}, function (err, numReplaced){
                        if(err){
...
router.get('/product/delete/:id', common.restrict, function(req, res){
	...
	// remove the article
    db.products.remove({_id: common.getId(req.params.id)}, {}, function (err, numRemoved){
        if(err){
            console.info(err.stack);
        }
...
router.post('/user/update', common.restrict, function(req, res){
    ...
    var isAdmin = req.body.user_admin === 'on' ? 'true' : 'false';

    // get the user we want to update
    db.users.findOne({_id: common.getId(req.body.userId)}, function (err, user){
        if(err){
            console.info(err.stack);
        }
        // if the user we want to edit is not the current logged in user and the current user is not
...
router.post('/user/insert', common.restrict, function(req, res){
    ...
    var urlParts = url.parse(req.header('Referer'));
    ...
    db.users.findOne({'userEmail': req.body.userEmail}, function (err, user){
        if(user){
...
// settings update
router.post('/settings/update', common.restrict, function(req, res){
    common.updateConfig(req.body);
    res.status(200).json({message: 'Settings successfully updated'});
});

// settings update
router.post('/settings/option/remove', common.restrict, function(req, res){
    var db = req.app.db;
    db.products.findOne({_id: common.getId(req.body.productId)}, function(err, product){
        if(err){
            console.info(err.stack);
        ...
        if(product.productOptions){
            var optJson = JSON.parse(product.productOptions);
            delete optJson[req.body.optName];

            db.products.update({_id: common.getImages(req.body.productId)}, {$set: {productOptions: JSON.stringify(optJson)}}, function (err, numReplaced){
                if(err){
                    console.info(err.stack);
                }
                if(numReplaced === 1){
...
// settings update
router.get('/settings/menu', common.restrict, function(req, res){
...
// settings page list
router.get('/settings/pages', common.restrict, function(req, res){
    var db = req.app.db;
    common.dbQuery(db.pages, {}, null, null, function(err, pages){
        if(err){
            console.info(err.stack);
        }
...
// settings pages new
router.get('/settings/pages/new', common.restrict, function(req, res){
...
router.get('/settings/pages/edit/:page', common.restrict, function(req, res){
    var db = req.app.db;
    db.pages.findOne({_id: common.getId(req.params.page)}, function (err, page){
        if(err){
            console.info(err.stack);
        }
        // page found
...
router.post('/settings/pages/update', common.restrict, function(req, res){
    var db = req.app.db;
    ...
    if(req.body.page_id){
        // existing page
        db.pages.findOne({_id: common.getId(req.body.page_id)}, function (err, page){
            if(err){
                console.info(err.stack);
            }
            if(page){
                db.pages.update({_id: common.getId(req.body.page_id)}, {$set: doc}, {}, function (err, numReplaced){
                    if(err){
                        console.info(err.stack);
                    }
        ...
        // insert page
        db.pages.insert(doc, function (err, newDoc){
            if(err){
...
router.get('/settings/pages/delete/:page', common.restrict, function(req, res){
    var db = req.app.db;
    db.pages.remove({_id: common.getId(req.params.page)}, {}, function (err, numRemoved){
        if(err){
...
// validate coupon
router.post('/coupon/validate', common.restrict, function(req, res){

});

// new menu item
router.post('/settings/menu/new', common.restrict, function(req, res){
    common.newMenu(req, res);
    res.redirect('/admin/settings/menu');
});

// update existing menu item
router.post('/settings/menu/update', common.restrict, function(req, res){
    common.updateMenu(req, res);
    res.redirect('/admin/settings/menu');
});

// delete menu item
router.get('/settings/menu/delete/:menuid', common.restrict, function(req, res){
    common.deleteMenu(req, res, req.params.menuid);
    res.redirect('/admin/settings/menu');
    return;
});

// We call this via a Ajax call to save the order from the sortable list
router.post('/settings/menu/save_order', common.restrict, function(req, res){
    common.orderMenu(req, res);
});
...
router.post('/api/validate_permalink', function(req, res){
    ...
    db.products.count(query, function (err, products){
        if(err){
            console.info(err.stack);
        }
        if(products > 0){
...
router.post('/product/published_state', common.restrict, function(req, res){
    ...
    db.products.update({_id: common.getId(req.body.id)}, {$set: {productPublished: req.body.state}}, {multi: false}, function (err, numReplaced){
        if(err){
...
router.post('/product/setasmainimage', common.restrict, function(req, res){
    ...
    db.products.update({_id: req.body.product_id}, {$set: {productImage: req.body.productImage}}, {multi: false}, function (err, numReplaced){
        if(err){
...
router.post('/product/deleteimage', common.restrict, function(req, res){
    ...
    db.products.findOne({_id: common.getId(req.body.product_id)}, function (err, product){
        if(err){
            console.info(err.stack);
        }
        if(req.body.productImage === product.productImage){
            // set the produt_image to null
            db.products.update({_id: common.getId(req.body.product_id)}, {$set: {productImage: null}}, {multi: false}, function (err, numReplaced){
                if(err){
                    console.info(err.stack);
                }
...
router.post('/file/upload', common.restrict, upload.single('upload_file'), function (req, res, next){
    ...
    if(req.file){
        ...
        var uploadDir = path.join('public/uploads', req.body.directory);
        ...
        var dest = fs.createWriteStream(path.join(uploadDir, file.originalname.replace(/ /g, '_')));
        ...
        db.products.findOne({_id: common.getId(req.body.directory)}, function (err, product){
            if(err){
                console.info(err.stack);
            }
            var imagePath = path.join('/uploads', req.body.directory, file.originalname.replace(/ /g, '_'));

            // if there isn't a product featured image, set this one
            if(!product.productImage){
                db.products.update({_id: common.getId(req.body.directory)}, {$set: {productImage: imagePath}}, {multi: false}, function (err, numReplaced){
                    if(err){
                        console.info(err.stack);
                    }
...
router.post('/testEmail', common.restrict, function(req, res){
    var config = common.getConfig();
